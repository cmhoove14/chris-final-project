...
sfx <- function(x, p, t) {
  ...
  W_tot = W_SAC*prop_SAC + W_adult*prop_adult
    ...
    #Estimate number of mated female worms in each population
      fem_Wt_SAC = 0.5*(Wt_SAC*phi_Wt_SAC)
      
      fem_Wu_SAC = 0.5*(Wu_SAC*phi_Wu_SAC)
      
      fem_Wt_adult = 0.5*(Wt_adult*phi_Wt_adult)
      
      fem_Wu_adult = 0.5*(Wu_adult*phi_Wu_adult)
  ...
  return(c(p$f_N * (1-N/p$C) * (S + E),   #Snail birth
           ...
           p$beta * (1-exp(-(sum(fem_Wt_SAC * f_Wgk(Wt_SAC, p$gam, k1) * p$m * ((p$H*p$prop_SAC)*p$cvrg_SAC) * p$v * p$u_SAC * p$rho_SAC,
                                 fem_Wu_SAC * f_Wgk(Wu_SAC, p$gam, k2) * p$m * ((p$H*p$prop_SAC)*(1-p$cvrg_SAC)) * p$v * p$u_SAC * p$rho_SAC,
                                 fem_Wt_adult * f_Wgk(Wu_adult, p$gam, k3) * p$m * ((p$H*p$prop_adult)*p$cvrg_adult) * p$v * p$u_adult * p$rho_adult,
                                 fem_Wu_adult * f_Wgk(Wu_adult, p$gam, k4) * p$m * ((p$H*p$prop_adult)*(1-p$cvrg_adult)) * p$v * p$u_adult * p$rho_adult))/N)) * S,  #Snail exposure
